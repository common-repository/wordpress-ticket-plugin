...
class iewPager {
    ...
    function page_links ($inactive_page_tag, $pager_url_last) {
        ...
        for ($i = $loop_start; $i <= $loop_finish; $i++) {
            if ($i == $this -> current_page) {
                $this -> page_links .= '<a '.$inactive_page_tag.' href="">'.$i.'</a>';
            }else{
                $this -> page_links .= '<a class="page-numbers" href="'.$this -> url.$i.$pager_url_last.'">'.$i.'</a>';
            }
        }
    }
    ...
    function previous_page ($previous_page_text, $pager_url_last) {
        if ($this -> current_page > 1) {
            $this -> previous_page = '<a class="page-numbers" href="'.$this -> url.($this -> current_page - 1).$pager_url_last.'">'.$previous_page_text.'</a>';
        }
    }
    ...
    function next_page ($next_page_text, $pager_url_last) {
        if ($this -> current_page < $this -> total_pages) {
            $this -> next_page = '<a class="page-numbers" href="'.$this -> url.($this -> current_page + 1).$pager_url_last.'">'.$next_page_text.'</a>';
        }
    }
    ...
    function first_page ($first_page_text, $pager_url_last) {
        if ($this -> current_page > 1) {
            $this -> first_page = '<a class="page-numbers" href="'.$this -> url.'1'.$pager_url_last.'">'.$first_page_text.'</a>'; // :)
        }
    }
    ...
    function last_page ($last_page_text, $pager_url_last) {
        if ($this -> current_page < $this -> total_pages) {
            $this -> last_page = '<a class="page-numbers" href="'.$this -> url.$this -> total_pages.$pager_url_last.'">'.$last_page_text.'</a>';
        }
    }
